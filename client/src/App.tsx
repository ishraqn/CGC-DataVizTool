...
const App: React.FC = () => {
	...
	const { uploadedFiles, currentFileIndex, setCurrentFileIndex, featureColors, featureVisibility, titlesById, toggleLegendVisibility, toggleTileLayer, legendLabels } = useToggle();
	...
	const handleDownload = async () => {
		try {
			...
			const response = await fetch("/api/v1/map/render-map", {
				...
				body: JSON.stringify({ filePath: selectedFile.path, fillColors: featureColors, visibileFeatures: featureVisibility, title: customTitle, tileLayer: toggleTileLayer, legendLabels: toggleLegendVisibility ? legendLabels : ""}),
			});
			if (!response.ok) {
				throw new Error(`HTTP error! Status: ${response.status}`);
			}
